...
if __name__ == "__main__":
    ...
    if fp16:
        ...
        scaler = None
    # 加载为训练结束的模型
    # modeldict_path=os.path.join("./",save_dir,"model.pth")
    modeldict_path=os.path.join("./",save_dir,"last_epoch_weights.pth")
    
    if os.path.exists(modeldict_path):    
      # model = torch.load(modeldict_path,map_location=device)
      model.load_state_dict(torch.load(modeldict_path,map_location=device))
